...
def get_answers(xml):
    ...
    try:
        if (
            xml.find("{http://www.imsglobal.org/xsd/ims_qtiasiv1p2}answer_tolerance")
            is not None
        ):
            tolerance = xml.find(
                "{http://www.imsglobal.org/xsd/ims_qtiasiv1p2}answer_tolerance"
            )
        for xml_var_set in xml.findall(
            ".//{http://www.imsglobal.org/xsd/ims_qtiasiv1p2}var_set"
        ):
            this_var_set = {
                "id": xml_var_set.get("ident"),
                "value": xml_var_set.find(
                    "{http://www.imsglobal.org/xsd/ims_qtiasiv1p2}answer"
                ).text,
                "text": "",
                "display": True,
                "correct": True,
            ...
            for xml_var_set_variable in xml_var_set.findall(
                ".//{http://www.imsglobal.org/xsd/ims_qtiasiv1p2}var"
            ):
                ...
                if this_var_set["text"] == "":
                    this_var_set["text"] = (
                        xml_var_set_variable.get("name")
                        + " = "
                        + xml_var_set_variable.text
                    )
                else:
                    this_var_set["text"] = (
                        this_var_set["text"]
